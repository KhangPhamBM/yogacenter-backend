        ...
        public async Task PaymentReminder()
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                foreach (var item in list)
                {
                    ...
                    {
                        emailService.SendEmail(item.User.Email, "NHAC NHO THANH TOAN", $"BAN CHUA THANH TOAN KIAAAA. THANH TOAN KHOA HOC THUOC LOP {item.Class.ClassName} GIA LA {item.Total}");
                    }
                }
            }
        }
